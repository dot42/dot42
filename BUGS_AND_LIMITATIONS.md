...
- ART does over-extensive checking if `lock`s are properly released in  functions. This can poses a problem in rare cases, e.g. when using locks in `async` methods. Such usages might provoke a dex2oat verification error with a message like "expected empty monitor stack". The workaround is to extract the locked region into it's own method, or, alternativly, use other synchronization primitives like `SemaphoreSlim` or something. 
